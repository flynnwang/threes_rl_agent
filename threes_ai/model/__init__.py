...
class ConvEmbeddingInputLayer(nn.Module):
    def __init__(
        ...
        merger_layers = []
        print(f'n_continuous_channels={n_continuous_channels}, n_embedding_channels={n_embedding_channels}')
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        for key, op in self.keys_to_op.items():
            ...
            if op == "embedding":
                ...
                embedding_outs[key] = out.permute(0, 3, 1, 2) # move channel into second column.
            elif op == "continuous":
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        if squeeze_excitation:
            ...
            self.squeeze_excitation = SqueezeExcitation(out_channels, squeeze_channels)
        else:
...
def create_model(observation_space, action_space,
                 embedding_dim = 32, hidden_dim = 128, n_blocks = 4):
  ...
  return BasicActorCriticNetwork(base_model, hidden_dim, action_space, reward_spec)
