...
def buffers_apply(
    ...
    func: Callable[[torch.Tensor], Any]) -> Union[Dict, torch.Tensor]:
  if isinstance(buffers, dict):
    return {
        key: buffers_apply(val, func)
        for key, val in copy(buffers).items()
    }
  else:
    return func(buffers)
...
def create_buffers(
    flags, obs_space, act_space: gym.spaces.Discrete,
    example_info: Dict[str, Union[Dict, np.ndarray, torch.Tensor]]) -> Buffers:
  ...
  specs = dict(
      ...
      policy_logits=dict(size=(t + 1, n, act_space.n), dtype=torch.float32),
      baseline=dict(size=(t + 1, n, 1), dtype=torch.float32),
      actions=dict(size=(t + 1, n), dtype=torch.int64),
  )
  buffers: Buffers = []
